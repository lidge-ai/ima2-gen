...
describe("NC — palette and compatibility boundary", () => {
  ...
  it("executes mapped compatibility and equivalent-handle cardinality rules", () => {
    const imageNode: any = { id: "image", type: "imageNode", position: { x: 0, y: 0 }, data: {} };
    const elementNodeValue: any = { id: "element", type: "elementReferenceNode", position: { x: 0, y: 0 }, data: { nodeType: "element-reference" } };
...
describe("EN — element node lifecycle", () => {
  ...
  it("executable — collectElementInputs walks upstream chains and dedupes", async () => {
    ...
    const nodes: any[] = [elementNode, missingNode, imageA, imageB];
    const edges = [
