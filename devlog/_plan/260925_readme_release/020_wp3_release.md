...
5. Approval 1 — release.yml `tag` job waits on `npm-stable` (`release.yml:180-187`). Approve with `gh api -X POST repos/lidge-jun/ima2-gen/actions/runs/<release run>/pending_deployments -F 'environment_ids[]=<npm-stable id>' -f state=approved -f comment=...` only after `cut` succeeded with the preview proof. The job then pushes main/dev/tag and dispatches publish.yml for `refs/tags/v3.21.0`.
